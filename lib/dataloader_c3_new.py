...
yolov3_config = {
                ...
                "anchor_num": 9  # each layer has (anchor_num) anchor
                 }
    ...
    def __getitem__(self, uid_item, wrap=20):
        ...
        for index, (f_map, f_name) in enumerate(zip(yolov3_config['feature_map'], yolov3_config['feature_name'])):
            if np.isnan(bboxs).sum():
                ...
                anchors_list = yolov3_config['anchors_list']
...
if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        print(arr.size(), anchor_dict['high_reso'].size(), anchor_dict['high_reso'].max())
